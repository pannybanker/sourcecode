                            ...
                            <div class="col-md-12">
                                <div class="panel_s">
                                    <div class="panel-body">
                                        ...
                                        <?php
                                        $where_all = array();
                                        $where_open = array('status'=>1);
                                        $where_total_paid = array('status'=>2);
                                        $where_total_not_paid_completely = array('status'=>3);
                                        $where_total_overdue = array('status'=>4);
                                        if(isset($project)){
                                            $where_all['project_id'] = $project->id;
                                            $where_open['project_id'] = $project->id;
                                            $where_total_paid['project_id'] = $project->id;
                                            $where_total_not_paid_completely['project_id'] = $project->id;
                                            $where_total_overdue['project_id'] = $project->id;
                                        }
                                        $total_invoices = total_rows('tblinvoices',$where_all);
                                        $total_open = total_rows('tblinvoices',$where_open);
                                        $total_paid = total_rows('tblinvoices',$where_total_paid);
                                        $total_not_paid_completely = total_rows('tblinvoices',$where_total_not_paid_completely);
                                        $total_overdue = total_rows('tblinvoices',$where_total_overdue);
