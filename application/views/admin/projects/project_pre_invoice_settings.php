     ...
     <div class="modal-body">
      <div class="row">
        <div class="col-md-12">
          ...
          <?php } ?>
          <?php if(count($billable_tasks) == 0 && count($not_billable_tasks) == 0){ ?>
            <p class="text-danger"><?php echo _l('invoice_project_nothing_to_bill'); ?></p>
          <?php } else { ?>
          <hr />
          <a href="#" onclick="slideToggle('#tasks_who_will_be_billed'); return false;"><?php echo _l('invoice_project_see_billed_tasks'); ?></a>
         ...
         <hr />
          <?php foreach($billable_tasks as $task){
            if($task['finished'] == 0){$not_finished_tasks_found = true;}
            ?>
            <div class="checkbox checkbox-primary">
              <input type="checkbox" name="tasks[]" value="<?php echo $task['id']; ?>" checked id="<?php echo $task['id']; ?>">
              <label for="<?php echo $task['id']; ?>"><?php echo $task['name']; ?> <?php if(total_rows('tbltaskstimers',array('task_id'=>$task['id'])) == 0){echo '<small class="text-danger">'._l('project_invoice_task_no_timers_found').'</small> | ';}; ?><small><?php if($task['finished'] == 1){echo _l('task_finished');}else{echo _l('task_not_finished');} ?></small></label>
            </div>
            <?php } ?>
