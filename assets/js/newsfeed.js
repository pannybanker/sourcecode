...
$(document).ready(function() {
    $('body').on('click','.open_newsfeed,.close_newsfeed',function(e){
        ...
        if(typeof($(this).data('close')) == 'undefined'){
            $.get(admin_url+'newsfeed/get_data',function(response){
...
function NewsFeedLightBox(e, id, postid) {
    ...
    $.get(admin_url + 'newsfeed/get_post_image_attachments/' + postid, function(response) {
        ...
        $('body').find('#newsfeed_images_modal').modal('show');
    }, 'json');

}
// Get the next image from post used in lightbox
function next_newsfeed_lightbox_image() {
    var current_image = $('body').find('#newsfeed_images_modal .modal-body img').data('id');
    var postid = $('body').find('#newsfeed_images_modal .modal-body img').data('postid');
    $('.prev').removeClass('hide');
    if (current_post_images.length == 1) {
        return;
    }
...
function add_lightbox_image(postid, id, filename) {
    $('body').find('#newsfeed_images_modal .modal-body').html('<img onclick="next_newsfeed_lightbox_image()" src="' + site_url + 'uploads/newsfeed/' + postid + '/' + filename + '" class="img img-responsive img-gallery" data-id="' + id + '" data-postid="' + postid + '">');
}
// Ping post to top
