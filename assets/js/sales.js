$(document).ready(function() {
    ...
    $('body').on('change', 'input[name="show_quantity_as"]', function() {
        var qty_heading = $(this).data('text');
        $('body').find('th.qty').html(qty_heading);
    });

    $('div.invoice').find('input[name="date"]').on('change', function() {
...
function add_item_to_table(data, itemid,merge_invoice) {
    ...
    $.when(get_taxes_dropdown_template(tax_name, data.taxname)).then(function(tax_dropdown) {
        ...
        if (billed_task != '') {
            $('#billed-tasks').append(hidden_input('billed_tasks[]', billed_task));
        }
        init_selectpicker();
        clear_main_values();
        reorder_items();
...
function calculate_total() {
    ...
    $('input[name="total_tax"]').val(_total_tax);
}
