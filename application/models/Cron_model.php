...
class Cron_model extends CRM_Model
{
                    ...
                    foreach ($staff as $member) {
                        if (has_permission('manageContracts', $member['staffid'])) {
                            add_notification(array(
                                'description' => _l('not_contract_expiry_reminder', substr($contract['description'], 0, 50)),
    ...
    private function recurring_expenses()
    {
        ...
        $this->load->model('staff_model');
        $recurring_expenses_email_data = _l('not_recurring_expense_cron_activity_heading') . ' - ' . _dt(date('Y-m-d H:i:s')) . '<br /><br />';
        $total_renewed                 = 0;
        foreach ($recurring_expenses as $expense) {
            ...
            if (date('Y-m-d') > $calculated_date_difference) {
                ...
                if ($insert_id) {
                    ...
                    $recurring_expenses_email_data .= _l('not_recurring_expenses_action_taken_from') . ': <a href="' . admin_url('expenses/list_expenses/' . $expense['id']) . '">' . $expense['id'] . '</a><br />';
                    $recurring_expenses_email_data .= _l('not_expense_renewed') . ' <a href="' . admin_url('expenses/list_expenses/' . $insert_id) . '">' . _l('id') . ': ' . $insert_id . '</a>';
    ...
    private function recurring_invoices()
    {
        ...
        $recurring_invoices_email_data = _l('not_recurring_invoices_cron_activity_heading') . ' - ' . _dt(date('Y-m-d H:i:s')) . '<br /><br />';
        $total_renewed                 = 0;
        foreach ($invoices as $invoice) {
            ...
            if ($months >= (int) $invoice['recurring']) {
                ...
                if ($id) {
                    ...
                    $recurring_invoices_email_data .= _l('not_action_taken_from_recurring_invoice') . ' <a href="' . admin_url('invoices/list_invoices/' . $invoice['id']) . '">' . format_invoice_number($invoice['id']) . '</a><br />';
                    // Get the new invoice
                    $invoice = $this->invoices_model->get($id);
                    $recurring_invoices_email_data .= _l('not_invoice_renewed') . ' <a href="' . admin_url('invoices/list_invoices/' . $id) . '">' . format_invoice_number($invoice->id) . '</a><br /><br />';
                }
            }
        }
            ...
            if ($task['duedate'] > date('Y-m-d')) {
                ...
                if ($diff <= $reminder_before) {
                    ...
                    foreach ($assignees as $member) {
                        ...
                        if($row){
                           add_notification(array(
                            'description' => _l('not_task_deadline_reminder', substr($task['name'], 0, 50)),
                           ...
                           $this->emails_model->send_email_template('task-deadline-notification', $row->email, false, false, $member['assigneeid'], false, false, false, $task['id']);
        ...
        foreach ($reminders as $reminder) {
            if (date('Y-m-d') >= $reminder['date']) {
                ...
                if ($reminder['rel_type'] == 'customer') {
                    ...
                    $description = _l('not_new_reminder_for', ucfirst($reminder['rel_type'])) . ' ' . $client->firstname . ' ' . $client->lastname;
                    ...
                    $description = _l('not_new_reminder_for', ucfirst($reminder['rel_type'])) . ' ' . $lead->name;
                }
                ...
                if ($reminder['notify_by_email'] == 1) {
                    $this->emails_model->send_simple_email($staff->email, _l('not_new_reminder_for', ucfirst($reminder['rel_type'])), $description . '<br />' . get_option('email_signature'));
                }
             ...
             if(strtotime($activity->dateadded) < strtotime('-1 WEEK')) {
                if(total_rows('tblstafftasks',array('rel_id'=>$project['id'],'rel_type'=>'project')) > 0){
                    ...
                    if($percent >= 100){
                        ...
                        $this->db->update('tblprojects',array('status'=>4));
        ...
        foreach ($estimates as $estimate) {
            if ($estimate['expirydate'] != NULL) {
                if (date('Y-m-d') > $estimate['expirydate']) {
                    ...
                    if ($this->db->affected_rows() > 0) {
                        $activity = '[CRON]' . _l('not_estimate_status_updated', array(
                            format_estimate_status($estimate['status']),
                            format_estimate_status(5)
                        ));
                        $this->estimates_model->log_estimate_activity($estimate['id'], $activity);
                    }
                }
            }
        }
            ...
            foreach ($emails as $email) {
                ...
                if ($total > 0) {
                    ...
                    if ($lead) {
                        ...
                        // Set message to seen to in the next time we dont need to loop over this message
                        $imap->setUnseenMessage($email['uid']);
                        $this->_notification_lead_email_integration(_l('not_received_one_or_more_messages_lead'), $mail, $lead->id);
                    ...
                    if ($insert_id) {
                        ...
                        // Set message to seen to in the next time we dont need to loop over this message
                        $imap->setUnseenMessage($email['uid']);
                        $this->_notification_lead_email_integration(_l('not_received_lead_imported_email_integration'), $mail, $insert_id);
                        $this->leads_model->log_lead_activity($insert_id, _l('not_received_lead_imported_email_integration'), '', true);
        ...
        if (isset($email['attachments'])) {
            foreach ($email['attachments'] as $key => $attachment) {
                ...
                if (fwrite($fp, $_attachment['content'])) {
                    ...
                    if ($attachment_id) {
                        $this->leads_model->log_lead_activity($leadid, _l('not_lead_imported_attachment'), '', true);
