...
class Mycon extends CI_Controller {
    ...
    public function index() {
        ...
        $strmtop="SELECT * FROM news_main where date(published_on)=curdate() AND news_category_ids='lmix' ORDER BY RAND() LIMIT 0,20";

        $resmtop=$this->db->query($strmtop)->result_array();

        $data['top_news']=$resmtop;

        $strmscroll="SELECT * FROM news_main where date(published_on)=curdate() AND news_category_ids='lmix' ORDER BY RAND() LIMIT 0,50";

        $resmscroll=$this->db->query($strmscroll)->result_array();

        $data['scroll_news']=$resmscroll;

        $strmfin="SELECT * FROM news_main where date(published_on)=curdate() AND news_category_ids='lfin' ORDER BY RAND() LIMIT 0,50";

        $resmfin=$this->db->query($strmfin)->result_array();

        $data['finance_news']=$resmfin;



        $strment="SELECT * FROM news_main where date(published_on)=curdate() AND news_category_ids='lnent' ORDER BY RAND() LIMIT 0,50";

        $resment=$this->db->query($strment)->result_array();

        $data['ent_news']=$resment;


        $data['next'] = 'world';
        $data['prev'] = '';
        $this->load->view('front/header');
        $this->load->view('front/bodylanding', $data);
        $this->load->view('front/footer');
    }

    public function set_landing_json() {


        $target_folder=date('Ymd');

        $urlarray=array(
             'lmix'=>'http://webhose.io/filterWebContent?token=efcbcf33-2afd-43be-90a0-ba396fb9f73d&format=json&ts=1507024557720&sort=crawled&q=language%3Aenglish%20thread.country%3AUS%20site_type%3Anews%20social.facebook.likes%3A%3E500%20(site_category%3Amedia%20OR%20site_category%3Aeastern_europe%20OR%20site_category%3Asports%20OR%20site_category%3Afinancial_news)',
              'lfin'=>'http://webhose.io/filterWebContent?token=efcbcf33-2afd-43be-90a0-ba396fb9f73d&format=json&ts=1507027687984&sort=crawled&q=language%3Aenglish%20thread.country%3AUS%20site_type%3Anews%20social.facebook.likes%3A%3E500%20(%20site%3Afinancialexpress.com%20OR%20site%3Acnbc.com%20OR%20site%3Acnn.com%20)',
            'lnent'=>'http://webhose.io/filterWebContent?token=efcbcf33-2afd-43be-90a0-ba396fb9f73d&format=json&ts=1507028568403&sort=crawled&q=language%3Aenglish%20thread.country%3AUS%20site_type%3Anews%20social.facebook.likes%3A%3E100%20(%20site%3Ahollywoodlife.com%20OR%20site%3Ahollywoodreporter.com%20OR%20site%3Ahollywood.com%20OR%20site%3Ahollywood-news.jp%20)'
        );
        ...
        if (!file_exists('./news_api/' . $target_folder . '/news_array/')) {
            ...
            foreach ($urlarray as $key => $val) {
                ...
                $tempArray [$key] = json_decode($resp);
                    ...
                    $fp = fopen('./news_api/' . date('Ymd') . '/'.$key.'.json', 'w');
    ...
    public function set_landing_news(){
    ...
    public function forgotpassword(){
        $eml=$this->input->post('email');
         $res=$this->db->get_where('user_account',array('user_email'=>$eml))->result_array();
      //  print_r($res);
        if(count($res)==0){
            echo 0;exit;
        }else{
            ...
            $this->db->where('user_email',$eml);
            $this->db->update('user_account',array('user_password_key'=>$pkey));
            $this->load->library('mcapi');

            $retval = $this->mcapi->lists();
            //$listID = "4566d821e2"; // obtained by calling lists();74496cb950
            $listID = "ac76b25ea4"; // obtained by calling lists();74496cb950
            $emailAddress = $eml;
            $lnk=base_url().'mycon/resetpassword/'.$pkey.'/'.MD5($res[0]['user_id']);
            $merge_vars = array(
                'FNAME' => $res[0]['user_name'],
                'LNAME' => $lnk
            );
    ...
    public function checkemail(){
        $res=$this->db->get_where('user_account',array('user_email'=>$this->input->post('email')))->result_array();

        if(!empty($res)){

            echo MD5($res[0]['user_id']);
    ...
    public function setnewpassword(){
        ...
        $pwdK=$this->input->post('pkey');

        $this->db->where('MD5(user_id)',$id);
        $this->db->where('user_password_key',$pwdK);

        $this->db->update('user_account',array('user_password'=>$pwd,'user_password_key'=>''));
        echo 'yes';exit;
...
//Retrieve the data from our text file.
  ...
  if($_POST['source']==1) {
    ...
    public function get_world_news(){
        ...
        $str='http://webhose.io/filterWebContent?token=efcbcf33-2afd-43be-90a0-ba396fb9f73d&format=json&ts=1506162780632&sort=crawled&q=language%3Aenglish%20site_type%3Anews%20(%20site_category%3Aenvironmental_safety%20OR%20site_category%3Aweather%20)';


        //Sports
        $str='http://webhose.io/filterWebContent?token=efcbcf33-2afd-43be-90a0-ba396fb9f73d&format=json&ts=1506163583235&sort=crawled&q=language%3Aenglish%20site_type%3Anews%20social.facebook.likes%3A%3E5%20(%20site_category%3Asports%20OR%20site_category%3Abaseball%20OR%20site_category%3Afootball%20)';

       //real estate
        $str='http://webhose.io/filterWebContent?token=efcbcf33-2afd-43be-90a0-ba396fb9f73d&format=json&ts=1506163879813&sort=crawled&q=language%3Aenglish%20site_type%3Anews%20social.facebook.likes%3A%3E5%20(%20site_category%3Areal_estate%20OR%20site_category%3Aconstruction%20)';
      // Art Life

        $str='http://webhose.io/filterWebContent?token=efcbcf33-2afd-43be-90a0-ba396fb9f73d&format=json&ts=1506165374260&sort=crawled&q=language%3Aenglish%20site_type%3Anews%20social.facebook.likes%3A%3E5%20(%20site_category%3Aentertainment%20OR%20site_category%3Amens_health%20OR%20site_category%3Aadventure_travel%20)';


          //markets

        $str='http://webhose.io/filterWebContent?token=efcbcf33-2afd-43be-90a0-ba396fb9f73d&format=json&ts=1506165969228&sort=crawled&q=language%3Aenglish%20site_type%3Anews%20social.facebook.likes%3A%3E5%20(%20site_category%3Astocks%20OR%20site_category%3Ainvesting%20)';

//Tech
        $str='http://webhose.io/filterWebContent?token=efcbcf33-2afd-43be-90a0-ba396fb9f73d&format=json&ts=1506166291088&sort=crawled&q=language%3Aenglish%20site_type%3Anews%20social.facebook.likes%3A%3E5%20(%20site_category%3Ascience%20OR%20site_category%3Atech%20)';

    //Business

        $str='http://webhose.io/filterWebContent?token=efcbcf33-2afd-43be-90a0-ba396fb9f73d&format=json&ts=1506167264912&sort=crawled&q=language%3Aenglish%20site_type%3Anews%20social.facebook.likes%3A%3E5%20(%20site_category%3Abusiness%20OR%20site_category%3Abusiness_software%20OR%20site_category%3Abusiness_travel%20)';


        //Economics

        $str='http://webhose.io/filterWebContent?token=efcbcf33-2afd-43be-90a0-ba396fb9f73d&format=json&ts=1506167881782&sort=crawled&q=language%3Aenglish%20site_type%3Anews%20social.facebook.likes%3A%3E5%20(%20site_category%3Afinancial_news%20OR%20site_category%3Afinancial_planning%20)';
