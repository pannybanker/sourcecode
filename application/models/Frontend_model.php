...
class Frontend_model extends CI_Model {
    ...
    function get_all_packages() {
        $strq = "select * from package_type order by pckg_priority asc";
        return $this->db->query($strq)->result_array();
    }
    
    function get_package_by_id($packege_id = null) {
        $this->db->where('pckgtype_id',$packege_id);
        return $this->db->get('package_type')->row_array();
    }

    function max_count()
    {
        $this->db->select('COUNT(pckgmeta_pckgid) as total');
        $this->db->group_by('pckgmeta_pckgid'); 
        $this->db->order_by('total', 'desc'); 
        return $this->db->get('package_meta')->row_array();
    }

    function get_package_meta($pckgid) {

        return $this->db->get_where('package_meta', array('pckgmeta_pckgid' => $pckgid))->result_array();
    }
    
    function get_user_package_meta($pckgid) {
        return $this->db->get_where('user_package', array('pay_packageid' => $pckgid,'pay_userid' => $_SESSION['user']['user_id']))->row_array();
    }
    ...
    public function get_all_countries() {
        ...
        $cntry = $this->db->get('tblcountries');
