...
function update_invoice_status($id)
{
    ...
    if ($CI->db->affected_rows() > 0) {
        ...
        logActivity('Invoice Status Updated ' . $_from_cron . ' [Invoice Number: ' . format_invoice_number($invoice->id) . ', From: ' . format_invoice_status($original_status, '', false) . ' To: ' . format_invoice_status($status, '', false) . ']', NULL);
        $activity = _l('invoice_activity_status_updated', array(
            format_invoice_status($original_status, '', false),
            format_invoice_status($status, '', false)
        ));
        $activity = $_from_cron . $activity;
        $CI->invoices_model->log_invoice_activity($invoice->id, $activity);
        return $status;
    }
    return false;
}
