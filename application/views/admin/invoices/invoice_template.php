...
<div class="row">
 <div class="col-md-12">
  <div class="single-option-buttons">
   <button type="submit" class="btn btn-info mleft10 text-right pull-right" <?php if((isset($invoice) && $invoice->status == 2 && !is_admin()) || (isset($invoice) && $invoice->status == 5)){echo 'disabled';} ?>>
     <?php echo _l('submit'); ?>
   </button>
 </div>
</div>
<div class="col-md-6 border-right">
 ...
 <?php echo render_select('clientid',$clients,array('userid','company'),'invoice_select_customer',$selected); ?>
 <div class="row">
   <div class="col-md-12">
  ...
  <div class="col-md-3">
    <div class="form-group" data-toggle="tooltip" data-title="<?php echo _l('invoice_task_item_project_tasks_not_included'); ?>">
      ...
      <option value=""></option>
        ...
        <option  value="<?php echo $task_billable['id']; ?>"<?php if($this->tasks_model->is_timer_started_for_task($task_billable['id'])){ ?>disabled class="text-danger important" data-subtext="<?php echo _l('invoice_task_billable_timers_found'); ?>" <?php } else {?> data-subtext="<?php echo _l('task_total_logged_time') .' '.format_seconds($this->tasks_model->calc_task_total_time($task_billable['id'])); ?>" <?php } ?>><?php echo $task_billable['name']; ?></option>
        <?php } ?>
      </select>
    </div>
  </div>
  <div class="col-md-6 text-right">
    <div class="mtop10">
     ...
     <div class="radio radio-primary radio-inline">
      <input type="radio" value="1" name="show_quantity_as" data-text="<?php echo _l('invoice_table_quantity_heading'); ?>" <?php if(isset($invoice) && $invoice->show_quantity_as == 1){echo 'checked';}else if(!isset($hours_quantity)){echo'checked';} ?>>
      <label for="<?php echo _l('quantity_as_qty'); ?>"><?php echo _l('quantity_as_qty'); ?></label>
    </div>
    <div class="radio radio-primary radio-inline">
      <input type="radio" value="2" name="show_quantity_as" data-text="<?php echo _l('invoice_table_hours_heading'); ?>" <?php if(isset($invoice) && $invoice->show_quantity_as == 2 || isset($hours_quantity)){echo 'checked';} ?>>
      <label for="<?php echo _l('quantity_as_hours'); ?>"><?php echo _l('quantity_as_hours'); ?></label>
    </div>
    <div class="radio radio-primary radio-inline">
      <input type="radio" value="3" name="show_quantity_as" data-text="<?php echo _l('invoice_table_quantity_heading'); ?>/<?php echo _l('invoice_table_hours_heading'); ?>" <?php if(isset($invoice) && $invoice->show_quantity_as == 3){echo 'checked';} ?>>
      <label for="<?php echo _l('quantity_as_hours'); ?>"><?php echo _l('invoice_table_quantity_heading'); ?>/<?php echo _l('invoice_table_hours_heading'); ?></label>
    </div>
  </div>
</div>
</div>
...
<tbody>
 ...
 <td>
   ...
   $select = '<select class="selectpicker display-block tax main-tax" data-width="100%" name="taxname" multiple>';
   $no_tax_selected = '';
   if($default_tax == ''){
  ...
  foreach($add_items as $item){
    ...
    if(isset($item['task_id'])){
      if(!is_array($item['task_id'])){
        $table_row .= form_hidden('billed_tasks[]',$item['task_id']);

      } else {
       foreach($item['task_id'] as $task_id){
        $table_row .= form_hidden('billed_tasks[]',$task_id);
      }
    }
...
<?php echo form_hidden('total_tax'); ?>
</div>
<div class="row">
