            ...
            if ($proposal->assigned != 0) {
                if ($proposal->assigned != get_staff_user_id()) {
                    add_notification(array(
                        'description' => _l('not_proposal_assigned_to_you', substr($proposal->subject, 0, 50)),
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($current_proposal->assigned != $proposal_now->assigned) {
                if ($proposal_now->assigned != get_staff_user_id()) {
                    add_notification(array(
                        'description' => _l('not_proposal_assigned_to_you', substr($proposal_now->subject, 0, 50)),
            ...
            if ($client == true) {
                foreach ($staff_proposal as $member) {
                    if (has_permission('manageSales', $member['staffid'])) {
                        add_notification(array(
                            'description' => _l('not_proposal_comment_from_client', substr($proposal->subject, 0, 50)),
    ...
    public function mark_action_status($status, $id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($client == true) {
                ...
                // Declined
                if ($status == 2) {
                    $message = _l('not_proposal_proposal_declined');
                } else if ($status == 3) {
                    $message = _l('not_proposal_proposal_accepted');
                    // Accepted
                } else {
                    $revert = true;
                }
