    ...
    if(count($results) > 0){
        ...
        <?php foreach($results as $_result){
            ...
            switch($heading){
                ...
                break;
                case 'leads':
                $data =  $data = '<a href="#" onclick="init_lead('.$_result['id'].');return false;">'.$_result['name'].'</a>';
                break;
                case 'tasks':
                $task_link = 'init_task_modal('.$_result['id'].'); return false;';
                $data =  $data = '<a href="#" onclick="'.$task_link.'">'.$_result['name'].'</a>';
                break;
                case 'contracts':
                ...
                break;
                case 'projects':
                $data =  $data = '<a href="'.admin_url('projects/view/'.$_result['id']).'">'.$_result['name'].'</a>';
                break;
            }
            ?>
            <li><?php echo $data; ?></li>
